...
public class AIPCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleTeam(CommandSender sender, String[] args) {
        ...
        switch (action) {
            ...
            default -> sender.sendMessage("§c未知 team 子命令: " + action);
        }
    }
    ...
    private void handleTask(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§c用法:");
            sender.sendMessage("§e/aip task assign <AI名> <gather|patrol|build|farm|escort>");
        ...
        switch (action) {
            case "assign" -> {
                if (args.length < 4) {
                    sender.sendMessage("§c用法: /aip task assign <AI名> <gather|patrol|build|farm|escort>");
                    return;
                }
                AIPlayer ai = plugin.getAiPlayerManager().get(args[2]);
                if (ai == null) {
                ...
                if (!List.of("gather", "patrol", "build", "farm", "escort").contains(type)) {
                    sender.sendMessage("§c未知任务类型: " + type + "（可选: gather, patrol, build, farm, escort）");
                    return;
                }
